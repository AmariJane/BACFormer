...
class MyDecoderLayer(nn.Module):
    def __init__(self, input_size, in_out_chan, heads, reduction_ratios, token_mlp_mode, n_class=9,
        ...
        self.layer_former_1 = MIS(out_dim, heads, reduction_ratios, token_mlp_mode)
        self.layer_former_2 = MIS(out_dim, heads, reduction_ratios, token_mlp_mode)
    ...
    def forward(self, x1, x2=None):
        if x2 is not None:
            ...
            if self.last_layer:
                out = self.last_layer(
                    self.layer_up(tran_layer_2).view(b, 4 * h, 4 * w, -1).permute(0, 3, 1, 2))
            else:
...
class BACFormer(nn.Module):
    def __init__(self, num_classes=9, token_mlp_mode="mix_skip", encoder_pretrained=True):
        ...
        heads = [1, 2, 5, 8]
        d_base_feat_size = 7
        in_out_chan = [[32, 64], [144, 128], [288, 320],
                       [512, 512]]

        dims, layers = [[64, 128, 320, 512],
                        [2, 2, 2, 2]]
        self.backbone = MyEncoder(224, dims, layers, token_mlp_mode)
